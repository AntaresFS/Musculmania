    ...
    return (
      <Card className="p-4 shadow-sm border-0 bg-white text-center">
        <Alert variant="info" className="mb-0">
          No hay datos de progreso disponibles aún. ¡Registra tu primer avance!
        </Alert>
      </Card>
    );
  }
      ...
      <Card.Body className="pt-3">
        <ResponsiveContainer width="100%" height={400}>
          ...
          >
            ...
            {data.some(item => item.weightKg !== null) && <Line type="monotone" dataKey="weightKg" stroke={colors.weight} name="Peso (kg)" activeDot={{ r: 8 }} />}
            {data.some(item => item.bicepsCm !== null) && <Line type="monotone" dataKey="bicepsCm" stroke={colors.biceps} name="Bíceps (cm)" activeDot={{ r: 8 }} />}
            {data.some(item => item.chestCm !== null) && <Line type="monotone" dataKey="chestCm" stroke={colors.chest} name="Pecho (cm)" activeDot={{ r: 8 }} />}
            {data.some(item => item.waistCm !== null) && <Line type="monotone" dataKey="waistCm" stroke={colors.waist} name="Cintura (cm)" activeDot={{ r: 8 }} />}
            {data.some(item => item.hipsCm !== null) && <Line type="monotone" dataKey="hipsCm" stroke={colors.hips} name="Caderas (cm)" activeDot={{ r: 8 }} />}
            {data.some(item => item.thighCm !== null) && <Line type="monotone" dataKey="thighCm" stroke={colors.thigh} name="Muslos (cm)" activeDot={{ r: 8 }} />}
            {data.some(item => item.glutesCm !== null) && <Line type="monotone" dataKey="glutesCm" stroke={colors.glutes} name="Glúteos (cm)" activeDot={{ r: 8 }} />}
          </LineChart>
        </ResponsiveContainer>
      </Card.Body>
    </Card>
